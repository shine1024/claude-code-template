...
### /apply-report — 분석 보고서 규칙 후보를 프로젝트에 반영

**용도**: `/analyze-report` 가 생성한 보고서의 규칙 후보를 사용자가 선택한 것만 `CLAUDE.md` / `.claude/rules/*.md` 에 반영하고, 매칭되는 `CLAUDE.local.md` 의 `## 공유 가능` 블록을 정리

| 항목 | 내용 |
|------|------|
| 호출 방법 | 프로젝트 루트에서 `/apply-report [YYYY-MM-DD]` (인자 생략 시 최신 보고서 자동 선택) |
| 적용 대상 | 보고서의 `## 신규 발견`, `## 개인 규칙 후보 승격 검토` 표의 행 |
| 적용 제외 | `## 사용자 가이드 제안`, `## 정보 보완 필요 항목` (참고용으로만 표시) |
| 컨텍스트 절약 | `parse-report.js` 가 표를 파싱하여 LLM 이 보고서 본문을 재해석하지 않음 |

**실행 절차**

| 단계 | 내용 |
|------|------|
| 1 | `parse-report.js` 로 보고서 파싱 (날짜 인자 또는 가장 최근 보고서) |
| 2 | 적용 가능한 후보에 일련번호 부여 (약 강도·사용자 가이드 제외) |
