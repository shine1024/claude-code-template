...
**용도**: Claude 응답을 콘솔 대신 브라우저(`http://127.0.0.1:<포트>/response.html`)에서 책 페이지 UI로 실시간 표시. 상세 동작은 [`basics/03 §5`](../../basics/03_claude-code-template_사용법.md) 참고.

| 이벤트 | 매처 | 동작 |
|--------|------|------|
| `UserPromptSubmit` | (없음) | 세션별 대기 마커(`viewer/.waiting-<sid>`) 생성 |
| `PostToolUse` | (없음) | 도구 호출마다 `viewer/response-<sid>.md` 점진 갱신 |
| `Stop` | (없음) | transcript 파싱 후 최종 응답 + 도구 호출 확정 |

**파일 구성**

| 파일 | 역할 |
|------|------|
| `prompt_hook.py` · `post_tool_hook.py` · `stop_hook.py` | 위 세 이벤트 훅 |
| `_index.py` | 세션 인덱스·포트 산출·transcript 파싱 공용 유틸 |

**동작 조건**

- `CLAUDE_VIEWER_ENABLED` 가 `true` 가 아니면 세 훅 모두 즉시 종료 (기본 비활성, opt-in)
- 훅 자체는 `settings.json` 에 기본 등록되어 있으므로, 활성화는 환경변수만 추가하면 된다
- 포트는 프로젝트 경로 해시로 자동 산출(20000–29999)되어 프로젝트 간 충돌하지 않는다

**활성화 방법**

`.claude/settings.local.json`(개인 설정, git 제외)에 환경변수를 추가한다.

```json
{
  "env": {
    "CLAUDE_VIEWER_ENABLED": "true"
  }
}
```
...
| `CLAUDE_VIEWER_ENABLED` | `.claude/settings.local.json` | 프로젝트별 | claude-viewer 브라우저 뷰어 활성화. 기본 비활성, `true` 설정 시에만 동작 |
