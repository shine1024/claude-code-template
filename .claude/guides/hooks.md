...
| `notify.js` | `Stop` 훅 — Slack DM 전송 (`lib/slack.js` 사용) |
...
| 이벤트 | 동작 |
|--------|------|
| `UserPromptSubmit` | 세션 1회, 로컬 `SYNC_HASH` ↔ 원격 `git HEAD` 비교 후 다르면 알림 |
...
| 파일 | 역할 |
|------|------|
| `slack.js` | Slack DM 전송 공용 모듈 — `lookupUserByEmail`, `sendDm`, `sendDmByEmail` 제공 |
