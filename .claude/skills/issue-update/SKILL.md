---
name: issue-update
description: 직전 커밋의 일감번호와 변경 내용을 분석하여 Redmine 일감 제목을 "[분류] 요약"으로 업데이트하고, 진척도(100%)·소요시간을 함께 반영한다. 상태는 `REDMINE_STATUS_ID_DONE` 환경변수가 설정된 경우에만 변경된다. "이슈 업데이트", "일감 수정", "/issue-update" 등의 요청에 사용한다. issue-new로 임시 생성한 일감을 정식 제목으로 반영하는 용도다.
---
...
1. 커밋 메시지에서 `#숫자` 패턴 (여러 개면 첫 번째)
2. 현재 브랜치명에서 `ing__issue__NNNN` 패턴 (`/issue-new`로 만든 브랜치 폴백) — `git rev-parse --abbrev-ref HEAD`
3. 둘 다 없으면 사용자에게 일감번호를 입력받는다.
...
커밋 메시지의 prefix `[xxx]`가 아래 셋 중 하나면 그대로 사용한다.
다른 prefix이거나 prefix가 없으면 변경 내용을 분석해 셋 중 하나로 매핑한다.
...
커밋 메시지의 제목에서 일감번호와 분류 prefix를 제거한 본문을 그대로 사용한다.
prefix가 없거나 메시지가 부족하면 변경 파일과 diff를 보고 한 문장(20자 내외)으로 요약한다.
...
> 커밋 메시지에는 `#일감번호`를 포함하지만 Redmine 제목에서는 제외한다 (Redmine UI에 일감번호가 별도 표시되므로 중복 표기 방지).
...
응답에서 `created_on`(ISO8601, UTC), `subject`, `status`, `done_ratio`를 보관한다.
...
응답의 `time_entry_activities[]`에서 `name`이 정확히 `"개발"` 또는 `"Development"`인 항목의 `id`를 사용한다 (한국어 이름 우선).
...
- 사용자가 분류·요약·시간 수정을 요청하면 해당 부분만 갱신 후 미리보기를 다시 출력한다.

---

## 6단계: API 호출

사용자가 확인하면 아래 두 요청을 순서대로 보낸다.
...
HTTP 상태 코드와 응답 본문을 그대로 보여주고, 가능한 원인을 한 줄로 설명한다. 일감 업데이트와 시간 등록 중 하나만 실패한 경우 어느 쪽이 성공/실패했는지 명시한다.
