---
name: commit-push
description: 작업 완료 후 변경분을 분석해 커밋 메시지(`#NNNN [분류] 요약`)를 자동 생성하고, 사용자 한 번의 확인으로 커밋·푸시·Redmine 일감 갱신(제목·진척도·소요시간)까지 일괄 처리합니다. "커밋해줘", "푸시해줘", "/commit-push" 등의 요청에 사용하세요. 일감번호는 브랜치명(`ing__issue__NNNN`) 또는 `.claude/cache/current_issue` 캐시에서 자동 추출합니다.
---
...
> 외부에서 푸시를 이미 한 후 일감만 갱신하려면 `/issue-update` 를 단독 호출하세요.
...
3. 둘 다 없으면 사용자에게 일감번호를 입력받습니다 (`#` 없이 숫자만)

`NNNN` 자릿수는 고정이 아니며 시스템마다 다릅니다 — 정규식은 `ing__issue__(\d+)` 와 같이 1자리 이상의 연속된 숫자를 매칭합니다.
...
기본은 **모든 변경분을 자동 스테이징**(`git add -A`)합니다. 단, 미리보기 단계에서 사용자가 다음을 지시하면 그에 따릅니다.

- "추적 안 된 파일은 빼" → `git add -u`
- "특정 파일만" → 사용자가 지정한 파일만 `git add <files>`
- ".gitignore 안 된 .env 같은 게 보이면 경고만 하고 멈춤"

미리보기 출력 시 **`?? 접두` 신규 파일 중 민감 패턴**(`.env`, `*credentials*`, `*secret*`, `*.key`, `*.pem`)이 보이면 사용자에게 경고 후 명시적 확인을 받습니다.
...
> 프로젝트별로 분류 셋이 다르면 해당 프로젝트의 `CLAUDE.md` 규칙을 우선합니다.

### 요약 (subject)

변경 파일과 diff를 보고 한 문장(공백 포함 30자 내외)으로 요약합니다. 너무 일반적인 표현(`개선`, `수정` 단독)은 피하고 가능한 한 구체적인 대상을 명시합니다.
...
기본은 비워둡니다. 사용자가 미리보기 단계에서 코멘트를 요청하면 subject 다음에 빈 줄을 두고 본문을 추가합니다.
...
> body는 커밋에만 들어가고 Redmine 제목엔 반영되지 않습니다.
...
응답에서 `created_on`, `subject`, `status`, `done_ratio`를 보관합니다.

### 새 제목

subject 라인에서 `#NNNN ` 부분을 제거한 나머지를 그대로 사용합니다.
...
확인 한 번으로 7단계 전체가 실행되므로, 모든 변경분을 한 화면에 정리합니다.
...
사용자가 다음과 같이 요청하면 해당 부분만 갱신해 다시 미리보기를 출력합니다.
...
> 분류·요약·body 외에 사용자가 임의의 subject 전체를 직접 지정할 수도 있습니다. 그 경우 `[분류]` 와 `요약` 을 역추출해 Redmine 제목을 동기화합니다.
...
사용자가 확인하면 아래 순서로 실행합니다. 어느 단계에서 실패하면 즉시 중단하고 7단계 결과 출력에서 어느 지점까지 성공했는지 명시합니다.
...
어느 단계까지 성공했는지 명확히 표시합니다.

```
⚠️ /commit-push 부분 완료
- ✅ 커밋: abc1234 ...
- ❌ 푸시 실패: {원인 — 예: rejected, non-fast-forward}
- ⏭️ Redmine 갱신 스킵 (푸시 실패로 보류)
