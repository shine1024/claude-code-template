...
선택 1·3·6은 별도 저장소의 core 프로젝트를 .jar 라이브러리로 참조합니다. `additionalDirectories` + `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD` 방식으로 연동합니다.
...
- **4단계**: **core 프로젝트 CLAUDE.md를 먼저 생성**한 후, 구현 프로젝트 CLAUDE.md를 생성합니다.
- **7단계**: 완료 안내에 `settings.local.json` 설정 방법을 포함합니다.

**패턴 B — 내부 멀티 모듈 의존 (uniflow 등):**
단일 저장소 내에서 common 모듈을 구현 모듈이 참조하는 구조입니다. 모듈 분석 순서로만 처리합니다.
...
core CONTEXT 파일의 `## 2. 분석 순서`가 있으면 그 순서대로 core 내부 모듈을 분석합니다.
...
- **core/common 라이브러리가 있는 프로젝트의 경우, 구현 프로젝트 CLAUDE.md에는 core가 이미 제공하는 내용(base class, 공통 유틸, 공통 아키텍처 등)을 반복 서술하지 않는다.** `additionalDirectories` + `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1` 설정으로 core CLAUDE.md가 자동 로드되므로, 구현 프로젝트 CLAUDE.md는 해당 프로젝트 고유의 도메인 로직·설정·규칙에만 집중한다.
...
core 경로가 입력된 경우 아래 형식으로 출력합니다. core 경로가 없으면 settings.local.json 안내 블록은 생략합니다.
...
📌 core 프로젝트 연동 설정
  core CLAUDE.md를 자동 로드하려면 구현 프로젝트의 .claude/settings.local.json 에 아래를 추가하세요.
  ...
  ───────────────────────────────────────────────
  {
    ...
    "permissions": {
      "additionalDirectories": [
        "{core 프로젝트 경로 — 절대 경로 또는 상대 경로}"
      ]
    }
  }
  ───────────────────────────────────────────────
  설정 후 Claude Code를 재시작하면 core CLAUDE.md 및 .claude/rules/*.md 가 자동 로드됩니다.
```
