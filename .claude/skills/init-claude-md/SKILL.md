...
단일 저장소 내에서 common 모듈을 구현 모듈이 참조하는 구조입니다. `@import`가 아닌 모듈 분석 순서로 처리합니다.
CONTEXT 파일의 **분석 순서** 필드를 읽어 그 순서대로 모듈을 분석·CLAUDE.md를 생성합니다. 필드가 없으면 pom.xml `<modules>` 선언 순서를 따릅니다.

### 2단계: CONTEXT 파일 읽기

**8번(기타)이 아닌 경우:** `$CLAUDE_CODE_TEMPLATE_PATH/template/{파일명}`을 읽어 아래 6개 섹션 전체를 추출합니다.

CONTEXT 파일은 **서비스 개요** 섹션만 관리합니다. 아래 필드를 추출합니다.

| 필드 | 내용 | 활용 |
|------|------|------|
| 서비스명, 목적, 주요 기능, 사용자 등 | 서비스 기본 정보 | CLAUDE.md Section 1 |
| **참조 라이브러리** | 의존하는 .jar 라이브러리 및 제공 내용 | CLAUDE.md Section 1, 분석 맥락 |
| **분석 순서** | 모듈·프로젝트 분석 순서 (`A → B` 형식) | 3단계 분석 순서 결정에 활용 |
