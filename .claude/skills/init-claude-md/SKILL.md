---
name: init-claude-md
description: 프로젝트를 분석하여 CLAUDE.md 파일을 생성한다. "CLAUDE.md 만들어줘", "프로젝트 초기화", "/init-claude-md" 등의 요청에 사용한다. 프로젝트 유형 선택 후 템플릿과 코드를 함께 분석하여 루트·하위 모듈 CLAUDE.md를 작성한다.
---

## 개요

`.claude/template/` 의 프로젝트별 CONTEXT 파일과 실제 코드 분석 결과를 종합해 루트 및 하위 모듈의 `CLAUDE.md` 초안을 생성한다.

## 사용 시점
- 신규 프로젝트에서 처음으로 Claude Code 환경을 구성할 때
- 트리거 표현: `CLAUDE.md 만들어줘`, `프로젝트 초기화`, `/init-claude-md`

## 실행 절차

### 1단계: 프로젝트 유형 선택

출력 경로는 현재 작업 디렉터리(`pwd`)를 사용한다.

아래 형식으로 출력하고 번호 입력을 요청한다.
...
프로젝트에 따라 두 가지 패턴으로 처리한다.

**패턴 A — 외부 저장소 의존 (UniWorks 구현 프로젝트):**
선택 4는 별도 저장소의 unidocu6 프로젝트를 .jar 라이브러리로 참조한다. `additionalDirectories` + `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD` 방식으로 연동한다.
아래 메시지를 출력하고 참조 프로젝트 경로 입력을 요청한다.
...
경로가 입력되면:
- **2단계**: 해당 프로젝트의 CONTEXT 파일도 함께 읽어 참조한다.
- **3단계**: 참조 프로젝트 코드를 먼저 분석한다 (3-0).
- **4단계**: **참조 프로젝트 CLAUDE.md를 먼저 생성**한 후, 구현 프로젝트 CLAUDE.md를 생성한다.
- **7단계**: 완료 안내에 `settings.local.json` 설정 방법을 포함한다.

**패턴 B — 내부 멀티 모듈 의존 (unidocu6·unidocu6-public-sap·uniflow 등):**
단일 저장소 내에서 core/common 모듈을 구현 모듈이 참조하는 구조다. 선택 1·2·5가 해당한다. 모듈 분석 순서로만 처리한다.
CONTEXT 파일의 `## 2. 분석 순서` 섹션을 읽어 그 순서대로 모듈을 분석·CLAUDE.md를 생성한다. 섹션이 없으면 pom.xml `<modules>` 선언 순서를 따른다.

### 2단계: CONTEXT 파일 읽기

**8번(기타)이 아닌 경우:** `.claude/template/{파일명}`을 읽어 아래 내용을 추출한다.

**`## 1. 서비스 개요`** 에서 아래 필드를 추출한다.
...
**`## 2. 분석 순서`** 가 있으면 그 순서대로 모듈·프로젝트를 분석한다. 없으면 pom.xml `<modules>` 선언 순서를 따른다.

참조 프로젝트 경로가 입력된 경우 (패턴 A — mobile-server) 해당 프로젝트의 CONTEXT 파일(`CLAUDE-TEMPLATE-{프로젝트}.md`)도 함께 읽어 참조한다.

CONTEXT 파일의 각 섹션은 CLAUDE.md에 그대로 복사하지 않는다. 3단계 코드 분석 결과와 종합하여 CLAUDE.md 고정 4섹션 구조에 녹여 작성한다.

> `.claude/template/{파일명}`이 없으면 아래 메시지를 출력하고 중단한다.
> ```
> [실패] .claude/template/ 폴더를 찾을 수 없습니다.
...
2단계에서 추출한 `## 2. 분석 순서` 값을 기준으로 이번 3단계 전체의 분석 순서를 확정한다.
...
확정된 순서대로 아래 3-0 ~ 3-5 분석을 반복 수행한다.

#### 3-0. 참조 프로젝트 분석 (패턴 A — unidocu6-mobile-server 전용)

2단계에서 참조 프로젝트(unidocu6) 경로가 입력된 경우, 구현 프로젝트 분석 전에 참조 프로젝트를 먼저 분석한다.
참조 프로젝트 CONTEXT 파일의 `## 2. 분석 순서`가 있으면 그 순서대로 내부 모듈을 분석한다.
...
> 3-0 분석이 완료되면 4단계에서 참조 프로젝트 CLAUDE.md를 먼저 생성한다. 멀티 모듈이면 5단계(모듈별 CLAUDE.md 생성)도 참조 프로젝트에 대해 먼저 수행한다.

#### 3-1. 기술 스택 파악

다음 파일을 읽어 기술 스택을 파악한다.
...
`src/main/java`, `src/main/resources`, `src/main/webapp` (Java) 또는 `src/` (React) 하위 주요 디렉터리를 탐색한다.
pom.xml에 `<modules>`가 있으면 각 모듈 디렉터리도 탐색한다.

**Java 소스가 없는 모듈 (client-common, clientcore 등):**
`src/main/java`가 없거나 비어 있으면 Java 분석을 건너뛰고, `src/main/resources/META-INF/resources/` 하위를 탐색한다.
...
코드에서 업무 도메인 정보를 추출한다. CONTEXT Section 2(도메인 지식)에 내용이 있으면 함께 참조하여 보완한다.
...
코드에서 아래 패턴을 탐색하여 발견되면 대응하는 규칙을 생성한다. CONTEXT Section 4·5에 내용이 있으면 함께 참조하여 보완한다.
...
**참조 프로젝트 경로가 입력된 경우 (패턴 A — mobile-server), 참조 프로젝트 CLAUDE.md를 먼저 생성한다.** 참조 프로젝트가 멀티 모듈이면 모듈별 CLAUDE.md도 함께 생성한다 (5단계 동일 적용). 이후 구현 프로젝트 CLAUDE.md를 생성한다.

아래 고정 스키마로 내용을 작성한다. 확인이 안 된 항목은 `{확인 필요}`로 표시한다.
...
pom.xml에 `<modules>`가 있으면 각 모듈을 순서대로 탐색하여 모듈별 CLAUDE.md를 생성한다.
...
> 루트 CLAUDE.md 내용을 반복하지 않는다.

### 6단계: 파일 저장

각 경로에 `CLAUDE.md`가 이미 존재하면 아래 메시지를 출력하고 확인을 요청한다.

```
{경로}/CLAUDE.md 파일이 이미 존재합니다. 덮어쓸까요? (예/아니오/전체예)
```

- **전체예**: 이후 모든 파일을 확인 없이 덮어쓴다.

### 7단계: 완료 안내

참조 프로젝트 경로가 입력된 경우 (패턴 A — mobile-server) 아래 형식으로 출력한다. 그렇지 않으면 settings.local.json 안내 블록은 생략한다.

```
CLAUDE.md 초안이 생성되었습니다:
{생성된 파일 경로 목록 — 참조 프로젝트 파일 먼저 나열}

[주의] 이 파일은 초안입니다. Claude가 틀린 결과를 낼 때마다 관련 내용을 추가·수정하세요.
