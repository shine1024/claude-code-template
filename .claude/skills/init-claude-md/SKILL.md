...
선택 1·3·6은 별도 저장소의 core 프로젝트를 .jar 라이브러리로 참조합니다. `@import` 방식으로 연동합니다.
...
선택 7(uniflow)은 단일 저장소 내에서 common 모듈을 구현 모듈이 참조하는 구조입니다. `@import`가 아닌 모듈 분석 순서로 처리합니다.
CONTEXT 파일의 **모듈 구성** 및 **분석 순서** 항목을 읽어 의존 그래프를 파악하고, common 모듈을 먼저 분석·CLAUDE.md 생성한 후 구현 모듈을 분석합니다.

| 프로젝트 | common 모듈 (먼저) | 구현 모듈 (나중) |
|---------|-----------------|--------------|
| uniflow | uniflow-server-common, uniflow-client-common | uniflow-web, uniflow-mobile |
...
- **core/common 라이브러리가 있는 프로젝트의 경우, 구현 프로젝트 CLAUDE.md에는 core가 이미 제공하는 내용(base class, 공통 유틸, 공통 아키텍처 등)을 반복 서술하지 않는다.** `@import`로 core CLAUDE.md가 자동 로드되므로, 구현 프로젝트 CLAUDE.md는 해당 프로젝트 고유의 도메인 로직·설정·규칙에만 집중한다.
