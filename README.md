...
Claude 응답을 콘솔 대신 브라우저(`http://localhost:19988/response.html`)에서 책 페이지 UI로 봅니다. `UserPromptSubmit`·`Stop` 훅이 자동으로 동작하므로 별도 실행 불필요 — `init.bat` 적용 후 바로 사용 가능합니다. 여러 Claude Code 세션을 동시에 띄워도 탭 UI 로 세션별 응답이 분리됩니다.
