# claude-code-template

사내 프로젝트에서 Claude Code를 빠르게 적용하기 위한 템플릿 및 가이드 모음입니다.

---

## 이 프로젝트의 두 가지 목적

| 목적 | 내용 |
|------|------|
| **템플릿 제공** | 프로젝트별 `CLAUDE.md`를 빠르게 적용할 수 있도록 완성된 템플릿 제공 |
| **기능 가이드** | Claude Code의 유용한 기능과 설정 방법을 팀 내 공유 |

---

## 1. CLAUDE.md 생성

Claude Code가 프로젝트에 맞는 코드를 생성하려면 **프로젝트 컨텍스트를 사전에 정의**해야 합니다.  
이를 위해 프로젝트 루트에 `CLAUDE.md` 파일을 작성합니다.

### 적용 절차

1. 아래 템플릿 목록에서 본인 프로젝트에 해당하는 파일을 찾는다
2. 해당 파일의 `---` 구분선 아래 내용을 프로젝트 루트에 `CLAUDE.md`로 생성한다
3. `.claude` 폴더를 프로젝트 루트에 복사한다
4. `CLAUDE.local.md`를 개인 환경에 맞게 작성 후 `.gitignore`에 추가한다
5. Claude가 틀린 결과를 낼 때마다 관련 내용을 `CLAUDE.md` 또는 `rules/`에 추가·수정한다

### 프로젝트별 템플릿 목록

| 파일 | 대상 프로젝트 |
|------|--------------|
| `template/CLAUDE-TEMPLATE-uniworks-pce.md` | unidocu6 (UniWorks PCE) |
| `template/CLAUDE-TEMPLATE-uniworks-pce-core.md` | unidocu6-core (UniWorks PCE Core) |
| `template/CLAUDE-TEMPLATE-uniworks-public.md` | unidocu6-public-sap (UniWorks Public) |
| `template/CLAUDE-TEMPLATE-uniworks-public-core.md` | unidocu6-public-sap-core (UniWorks Public Core) |
| `template/CLAUDE-TEMPLATE-uniworks-pce-mobile.md` | unidocu6-mobile (UniWorks PCE Mobile 프론트엔드) |
| `template/CLAUDE-TEMPLATE-uniworks-pce-mobile-server.md` | unidocu6-mobile-server (UniWorks PCE Mobile 백엔드) |
| `template/CLAUDE-TEMPLATE-uniflow.md` | uniflow |

### 디렉토리 구조

```
프로젝트루트/
├── CLAUDE.md                        ← Claude Code 프로젝트 지침
├── CLAUDE.local.md                  ← 개인 설정 (.gitignore에 추가)
└── .claude/
    ├── settings.json                ← 권한·모델 등 기술 설정
    ├── rules/                       ← 관심사별 규칙 (세션 시작 시 자동 로드)
    ├── skills/                      ← 커스텀 슬래시 커맨드
    └── hooks/                       ← 이벤트 훅
```

---

## 2. 훅 (Hooks) 설정

특정 이벤트 발생 시 자동으로 실행되는 스크립트입니다.  
현재 **Slack 알림 훅**이 기본 제공됩니다. (세션 시작·종료 시 Slack DM 전송)

### 설정 절차

**① Bot Token 설정**

```bash
cp .claude/hooks/slack-notify/notify.config.example.json \
   .claude/hooks/slack-notify/notify.config.json
```

`notify.config.json`에 Slack Bot Token을 입력합니다.

```json
{
  "slackBotToken": "xoxb-YOUR_SLACK_BOT_TOKEN"
}
```

> `notify.config.json`은 `.gitignore`에 추가해 커밋하지 않습니다.

**② 수신 이메일 설정**

전역 설정 파일(`~/.claude/settings.json`)에 알림을 받을 Slack 이메일을 추가합니다.

```json
{
  "env": {
    "SLACK_USER_EMAIL": "your@email.com"
  }
}
```

**③ 알림 비활성화 (선택)**

특정 프로젝트에서 알림을 끄려면 `.claude/settings.local.json`에 추가합니다.

```json
{
  "env": {
    "SLACK_NOTIFY_ENABLED": "false"
  }
}
```

> 상세 내용: [`guides/hooks.md`](guides/hooks.md)

---

## 3. 스킬 (Skills) 설정

`/스킬명`으로 호출하는 커스텀 커맨드입니다.  
현재 **세션 회고 스킬**이 기본 제공됩니다.

### /session-log — 세션 회고 기록

세션 대화를 분석해 회고 초안을 작성하고 Google Sheets에 제출합니다.

**설정 절차**

`CLAUDE.local.md`에 아래 항목을 추가합니다.

```markdown
## 세션 회고 설정 (/session-log)

- 세션회고_이름: [이름]
- 세션회고_시스템명: [프로젝트명]
- 세션회고_SCRIPT_URL: [Google Apps Script 배포 URL]
```

**사용 방법**

세션 종료 전 Claude에게 `/session-log`를 입력합니다.

> Google Apps Script 초안: `.claude/skills/session-log/scripts/google-apps-script.js`  
> 상세 내용: [`guides/skills.md`](guides/skills.md)

---

## 참고

- 코드 스타일 규칙: `.claude/rules/`
- 예시 rules: `java-style.md`, `js-style.md`, `sql-style.md`
