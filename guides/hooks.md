...
| 파일 | 역할 |
|------|------|
| `notify.config.json` | Slack Bot Token 저장 (git 제외) |
...
`notify.config.example.json`을 복사해 `notify.config.json`을 생성하고 Slack Bot Token을 입력합니다.
...
| 환경변수 | 설정 위치 | 범위 | 설명 |
|----------|-----------|------|------|
| `SLACK_USER_EMAIL` | `~/.claude/settings.json` | 전역 (PC 전체) | 알림을 수신할 본인 Slack 이메일 |
| `SLACK_NOTIFY_ENABLED` | `.claude/settings.local.json` | 프로젝트별 | `false` 설정 시 해당 프로젝트 알림 비활성화 |

**전역 설정 예시** (`~/.claude/settings.json`)
```json
{
  "env": {
