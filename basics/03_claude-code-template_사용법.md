...
1. **원격 HEAD 해시 비교** — `git ls-remote` 로 원격 HEAD 만 먼저 조회. 로컬 `SYNC_HASH` 와 같으면 clone·복사를 모두 건너뛰고 종료.
2. **다를 경우에만 clone → 복사 → 해시 갱신 → 변경 이력 Slack DM** 순으로 진행.

| 항목 | 동작 |
|------|------|
| `.claude/hooks/` `.claude/rules/` `.claude/skills/` | template의 최신본으로 **덮어쓰기** (변경 있을 때만) |
...
| Slack DM | `SLACK_BOT_TOKEN`·`SLACK_USER_EMAIL` 설정 시 본인에게 `oldHash..HEAD` 구간의 `CHANGES.md` 신규 항목 전송 (옵션) |
